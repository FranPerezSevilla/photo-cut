...
void main() {
  testWidgets(
    'preview is one tap away and preserves settings without rebuilding',
    (WidgetTester tester) async {
      ...
      expect(
        loadCount,
        1,
        reason: 'unchanged settings should reuse the current PDF',
      );
    },
  );

  testWidgets(
    'invalid settings keep preview reachable but do not show stale PDF',
    (WidgetTester tester) async {
      int loadCount = 0;
      await tester.pumpWidget(
        MaterialApp(
      ...
      expect(
        find.byKey(const Key('step-one-preview-unavailable')),
        findsOneWidget,
      );
...
PrintDocument _document(PrintJobConfiguration configuration) {
  return PrintDocument(
    ...
    filename:
        'photo-cut-${configuration.photoWidth.inMillimetres.round()}mm.pdf',
